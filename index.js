...
function customEntries(obj) {
  ...
  const methodEntries = Object.entries(obj);
...
function customObjectFromEntries(entries) {
  ...
  entries.map((key, value) => {
     if (typeof [key, value] === 'number') {
      [key, value] = String(entries[key, value]);
     }  
  })
  const MethodFromEntries = Object.fromEntries(entries);
  return MethodFromEntries;
...
function keysAndValues(obj) {
  ...
  return Array(arraykey, arrayvalues);
...
function filterObjectsByKey(arr, key) {
  ...
  const arrayObject = arr.filter((obj) => 
...
function getObjectValuesSum(obj) {
  ...
  const result = allValues.reduce((accumulator, currentValue) => accumulator + currentValue);
  return result;
  // Повертаємо суму
}
